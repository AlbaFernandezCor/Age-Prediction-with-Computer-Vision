...
class OldSchoolMethod():
    ...
    def run(self, train_dataset, test_dataset, model_loaded = None, df_type='age'):
        if model_loaded:
            model = self.model_load()
            print('Preparant les dades de test...')
            X_test_preprocess, y_test_preprocess = self.preprocesing_Xy(test_dataset, df_type)
            ...
            X_train_preprocess, y_train_preprocess = self.preprocesing_Xy(train_dataset, df_type)
            print('Preparant les dades de test...')
            X_test_preprocess, y_test_preprocess = self.preprocesing_Xy(test_dataset, df_type)
    ...
    def preprocesing_Xy(self, df, df_type):
        X_list = []
        y_list = []
        img_log = ceil(len(df)*0.05)
        print(img_log)
        for index in range(len(df)):
            descriptors = self.extract_sift_features(np.array(Image.open(df.iloc[index]['image'])))
            if descriptors is not None:
                X_list.append(np.array(np.concatenate(descriptors, axis=0).tolist()))
                y_list.append(df.iloc[index][df_type])
                if (index % img_log == 0):
